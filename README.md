...
Because ChatGPT's Cloudflare front door challenges true headless Chrome, background work uses a real native Chrome instance. On macOS the bridge uses LaunchServices background and hidden launch options, then a native visibility guard scoped to that Chrome process. This handles Chrome revealing itself when tabs are created and macOS clamping off-screen coordinates. The guard checks every 250 ms only while the worker browser is open; a brief OS transition is still possible. Codex attaches afterward through a random loopback-only DevTools port. A deliberately visible inspection omits the background flag and can bring the controlled window on-screen.
