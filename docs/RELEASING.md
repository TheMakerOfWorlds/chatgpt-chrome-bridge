...
1. Make and review the change. Update `package.json`, the root package version in `package-lock.json`, and the base version in `.codex-plugin/plugin.json` to the same new semantic version, such as `0.2.4`. Keep release versions unique; never replace the contents of an existing version.
